   ...
   if ((bOk) && (pNewNode->bDirectory))
	 ...
	 char** newFiles = copyFileList(pFiles,fileCount);
	     ...
	     if (pNewNode->childFiles == NULL)
	         {
                 printf("Error allocating child array for directory '%s'\n",
	            filename);
...
char* getFullPath(FILE_ENTRY_T* pTreeNode)
{
    ...
    FILE_ENTRY_T** stack = calloc(pTreeNode->level+1,sizeof(FILE_ENTRY_T*));
    if (stack == NULL)
	return NULL;
...
int main(int argc, char* argv[])
{
   ...
   while (1)
      {
      printf("Enter name of file to search for (without path, RETURN to exit): ");
