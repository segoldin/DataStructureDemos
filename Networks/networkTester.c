...
int main(int argc, char *argv[])
{
	...
	case 2:		/* add vertex */
		...
		{
		    printf
			(">>> Vertex with key |%s| already exists in graph\n",
			 key);
		}
		else
		{
	...
	case 3:		/* add edge */
		...
		{
		    printf
			(">>> Vertex doesn't exist or memory allocation error!\n");
		}
	    }
	    break;
	...
	case 8:		/* do Dijkstra's algorithm to find shortest path */
	    ...
	    input[strlen(input) - 1] = '\0';
	    sscanf(input, "%s", keystring2);
	    weight = printShortestPath(keystring1, keystring2);
	    if (weight == -1)
		printf
		    (">>> At least one key value does not exist in network\n");
	    else if (weight == -2)
		printf
		    (">>> Network must be directed to use Dijkstra's algorithm\n");
	    else if (weight == -3)
		printf(">> Graph has some negative weights\n");
	    else if (weight == -4)
